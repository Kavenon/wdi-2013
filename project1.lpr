...
var
  ...
  spup,spdown,long,short,rzepa, bron,bulletTex,heart,mTex,sTex,statTex,wybTex,endTex,maTex,catTex: zglPTexture;
  bonusWide :integer = 50;
  brickW :integer = 80;
  brickH :integer = 15;
  bonusSpeed :single = 1.25;
  points:integer =0;
  username,easter:string;
  ...
  wrecking :boolean = false;
  catX : integer =0 ;
  catY :integer =0;
  easterinput : boolean = false;
procedure colSound;
begin
...
procedure wreckingSound;
begin
  audio := snd_PlayFile( 'data/wrecking.ogg');
end;
...
begin
  ...
  bulletTex := tex_LoadFromFile( 'data\bullet.png');
   catTex := tex_LoadFromFile( 'data\cat.png');
...
begin
   ...
   if(wrecking = true) then ssprite2d_Draw(catTex, catX, 0, 200, 600, 0);
...
var nowy,x :player;
  i,j,k:integer;
begin
  nowy.username := TrimLeft(username);
  nowy.points := points;
...
begin
  ...
  begin
       if(easterinput = false) then key_BeginReadText( easter, 16 );
       easterinput := true;
...
begin
      ...
      begin
           easter := key_GetText();
           if(easter = 'harlem') then
           begin
             dotheharlem :=true;
           end
           else if(easter = 'cat') then
           begin
             wrecking := true;
           end;
...
begin
     ...
     begin
          if harlemcount = 0 then  harlemSound;
           ...
           if(harlemcount < 1860) and (harlemcount > 0) then
           begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if(harlemcount MOD 2 = 0) then bricks[i].color := random(3);
              end;
            end;
     ...
     else if (wrecking = true) then
     begin

          if(catX = 0) then wreckingSound;
